	...
	function testing(&$bean, $event, $arguments)
    {
      	...
      	foreach ($locations as $loc) 
        {
          	...
          	$loc_bean = BeanFactory::getBean('fyn_locations',$loc->id);
     		    $orders=$loc_bean->get_linked_beans('fyn_locations_op_orders_1','op_Orders');

         		foreach ($orders as $order) 
         		{
           			echo "<br><br>Order ID:";print_r($order->id);echo"<br><br>";
           			$order_bean = BeanFactory::getBean('op_Orders',$order->id);

           			$order_bean->load_relationship('fyn_routes_op_orders_1fyn_routes_ida');
              	$order_bean->fyn_routes_op_orders_1fyn_routes_ida->add($bean->id);
         		}
